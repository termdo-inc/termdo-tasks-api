...
@_router.get("/accounts/{account_id}/tasks", status_code=status.HTTP_200_OK)
async def get_tasks(
...
@_router.post("/accounts/{account_id}/tasks", status_code=status.HTTP_201_CREATED)
async def post_task(
    account_id: int = Path(gt=0),
...
@_router.get("/accounts/{account_id}/tasks/{task_id}", status_code=status.HTTP_200_OK)
async def get_task(
    account_id: int = Path(gt=0),
...
@_router.put("/accounts/{account_id}/tasks/{task_id}", status_code=status.HTTP_200_OK)
async def put_task(
    account_id: int = Path(gt=0),
    task_id: int = Path(gt=0),
    task: TasksRequest = Body(),
...
@_router.delete("/accounts/{account_id}/tasks/{task_id}", status_code=status.HTTP_204_NO_CONTENT)
